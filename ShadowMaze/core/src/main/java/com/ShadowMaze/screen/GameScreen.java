...
//        offsetY = 0;
    ...
    public void spawnEnemiesFromWalkableTiles(Map map, int numEnemies) {
        ...
        for (int i = 0; i < obj.length && placed < numEnemies; i++) {
            if (obj[i] == null) { // Ch? ??t n?u ch?a cÛ object
...
// C?p nh?t tr?ng th·i phÌm E ?? b?n
        if (Gdx.input.isKeyPressed(Input.Keys.E)) {
            if (!wasEPressedLastFrame && isSkillAvailable) { // ? Ki?m tra thÍm isSkillAvailable
                ...
                if (dir != Direction.IDLE) {
                    ...
                    isSkillAvailable = false; // ? KhÙng cho b?n ti?p
                }
            }
