...
/**
 * The Knight class represents the main player character in the game.
 * It handles movement, collision detection, animation, stamina and HP logic,
 * as well as interactions with objects and map transitions.
 */
...
//        }
        ...
        if (!collisionOn) {
            switch (currentDirection) {
                case UP -> positionY -= speed;
                case DOWN -> positionY += speed;
                case LEFT -> positionX -= speed;
                case RIGHT -> positionX += speed;
            }
    ...
    public void dispose() {
        ...
        for (TextureRegion region : moveUp.getKeyFrames()) region.getTexture().dispose();
        for (TextureRegion region : moveDown.getKeyFrames()) region.getTexture().dispose();
        for (TextureRegion region : moveLeft.getKeyFrames()) region.getTexture().dispose();
        for (TextureRegion region : moveRight.getKeyFrames()) region.getTexture().dispose();
    }
}
